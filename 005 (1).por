/*
Prioridades
Este exemplo demonstra a prioridade das operações aritiméticas no Portugol. As operações de multiplicação (*), divisão (/) e módulo (%) tem prioridade sobre as operações de soma (+) e subtração (-). Além disso, o exemplo demonstra como os parênteses podem ser utilizados para alterar esta prioridade, fazendo com que uma operação de soma ocorra antes de uma operação de multiplicação.
*/
